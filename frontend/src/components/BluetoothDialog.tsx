...
export default function BluetoothDialog() {
  ...
  const submit = async () => {
    ...
    const result = await printerContext.actions.addBluetoothPrinter(mac, name);
    ...
    >
      ...
      {devices.length > 0 && (
        <div className="mb-4 border border-gray-200 rounded-lg overflow-hidden">
          <div className="text-xs text-gray-500 px-3 pt-2 pb-1 font-medium uppercase tracking-wide bg-gray-50">
            Paired Devices
          </div>
              ...
              return (
                ...
                >
                  {device.device === "printer" ? (
                    <PrinterIcon className="w-4 h-4 shrink-0 text-gray-500" />
                  ) : (
                    <BluetoothIcon className="w-4 h-4 shrink-0 text-gray-500" />
                  )}
