  ...
  <div>
    <div
      ...
      <div v-if="printers.length || unavailablePrinters.length" class="p-6 overflow-y-auto max-h-[60vh]">
        <ul class="divide-y divide-gray-300">
          <li v-for="printer in printers" :key="printer.id" class="text-left first:pt-0 py-6 last:pb-0 relative">
            <div class="flex items-center gap-2">
              ...
              <span v-if="printer.isLAN || String(printer.name).startsWith('PDF_NET_')" @click="removePrinter(printer)"
        ...
        >
          <span
            v-if="String(printer.name).startsWith('PDF_NET_')"
...
async function removePrinter(printer) {
  if (!printer.lanIp && !String(printer.name).startsWith("PDF_NET_")) return
