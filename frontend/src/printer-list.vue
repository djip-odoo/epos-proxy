...
async function updatePrinters() {
  ...
  try {
    ...
    // Update in-place to avoid unmounting/remounting v-for child components
    printers.value.splice(0, printers.value.length, ...res.printers)
    unavailablePrinters.value.splice(0, unavailablePrinters.value.length, ...res.unavailablePrinters)
