<template>
  <button @click="showLANDialog = true" id="lan-access-btn" title="LAN Access Settings"
    class="absolute top-3 left-3 w-12 h-12 flex items-center justify-center rounded-full shadow-lg transition-all duration-300"
    :class="lanEnabled
      ? 'bg-odoo text-white shadow-odoo/30'
      : 'bg-odoo/50 text-white shadow-gray-300/30'">
...
onMounted(async () => {
  ...
  try { EventsOn('open-lan-settings', () => { showLANDialog.value = true }) } catch { }
})
...
async function handleToggle() {
  ...
  error.value = null
  loading.value = true
  loadingMessage.value = enabled ? 'Applying firewall rule…' : 'Removing firewall rule…'

  try {
    if (enabled) {
      await EnableLANAccess()
      emit('notify', 'LAN access enabled. Server is now accessible on your local network.', 'success')
    } else {
    ...
    const message =
      typeof err === 'string'
        ? err
        : err?.message || 'Failed to update LAN access settings.'
