    ...
    <transition enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0"
      ...
      <div v-if="showBluetoothDialog" class="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4">
        ...
        <div class="relative bg-white rounded-2xl w-full max-w-sm shadow-xl overflow-hidden p-6">
          ...
          <div v-if="dependencies.some(d => !d.installed)"
            ...
            <div class="space-y-2">
              <div v-for="dep in dependencies.filter(d => !d.installed)" :key="dep.name"
          ...
          <div v-if="devices.length" class="mb-4 border border-gray-200 rounded-lg overflow-hidden">
            ...
            <ul class="divide-y divide-gray-100 max-h-44 overflow-y-auto">
              <li v-for="device in devices" :key="device.mac" @click="selectDevice(device)"
                ...
                <span v-if="selectedMac === device.mac" class="w-4 h-4 shrink-0 text-blue-500"
...
import { AddBluetoothPrinter, ScanBluetoothPrinters, CheckBluetoothDependenciesAndOs } from '../../wailsjs/go/main/App'
...
async function checkDeps() {
  try {
    const res = await CheckBluetoothDependenciesAndOs()
    dependencies.value = res.deps || []
    os.value = res.os || null
  } catch (err) {
