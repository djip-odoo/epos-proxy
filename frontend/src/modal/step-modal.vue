<template>
  <teleport to="body">
    <transition enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0"
      ...
      <div v-if="visible" class="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4">
        ...
        <div class="relative bg-white rounded-2xl w-full max-w-md shadow-xl overflow-hidden">
          ...
          <div class="overflow-hidden transition-[height] duration-300 ease-in-out" :style="{ height: contentHeight }">
            <transition enter-active-class="transition duration-300 ease-out" enter-from-class="opacity-0 translate-x-4"
              ...
              <div :key="currentStep" ref="contentRef" class="px-5 pt-2 pb-2 overflow-y-auto" style="max-height: 70vh">
                <h3 class="font-semibold text-lg text-gray-900 mb-1.5">{{ activeSteps[currentStep].title }}</h3>
                <p class="text-gray-500 whitespace-pre-line" v-html="formatDesc(activeSteps[currentStep].desc)"></p>

                <a v-if="activeSteps[currentStep].link" :href="activeSteps[currentStep].link" target="_blank"
                  @click.prevent="BrowserOpenURL(activeSteps[currentStep].link)"
                  ...
                  {{ activeSteps[currentStep].linkLabel }}
                </a>
                <img v-if="activeSteps[currentStep].image" :src="activeSteps[currentStep].image"
                  :alt="activeSteps[currentStep].title" class="max-w-full mt-3 rounded-lg" />
                <div v-if="activeSteps[currentStep].codes" v-for="(code, index) in activeSteps[currentStep].codes"
          ...
          <div class="px-5 pb-5 pt-3 flex gap-2">
            ...
            <template v-if="activeSteps[currentStep].actions?.length">
              <button v-for="action in activeSteps[currentStep].actions" :key="action.label"
                @click="handleAction(action)"
                class="flex-1 py-2 rounded-lg bg-odoo text-white hover:bg-odoo-dark cursor-pointer transition-colors">
                {{ action.label }}
              </button>
            </template>

            <template v-else>
              <button v-if="currentStep < activeSteps.length - 1"
                class="flex-1 py-2 rounded-lg bg-odoo text-white hover:bg-odoo-dark whitespace-nowrap cursor-pointer transition-colors"
                @click="next(activeSteps.length)">
                Next
              </button>
...
const { show: docsShow, steps: docsSteps, closeDocs } = useDocs()

// Hook mode when modelValue prop is not explicitly provided.
const isHookMode = computed(() => props.modelValue === undefined)
const visible = computed(() => isHookMode.value ? docsShow.value : props.modelValue)
const activeSteps = computed(() => isHookMode.value ? docsSteps.value : (props.steps ?? []))

function closeModal() {
  if (isHookMode.value) closeDocs()
  else emit('update:modelValue', false)
}

const { currentStep, next, back, close } = useStepModal(
  () => visible.value, (event, val) => { if (event === 'update:modelValue' && !val) closeModal() },
)
...
function handleAction(action) {
  if (action.goto) {
    const stepIndex = activeSteps.value.findIndex(
      step => step.id === action.goto
    )

    if (stepIndex !== -1) {
      currentStep.value = stepIndex
    } else {
      console.warn(`Step id "${action.goto}" not found`)
    }
