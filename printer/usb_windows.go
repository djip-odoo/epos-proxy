...
func listSystemPrinters() ([]SystemUsbPrinter, error) {
	...
	for _, p := range printersWMI {
		...
		isLAN := strings.HasPrefix(p.Name, SYSTEM_PDF_NETWORK_PRINTER_PREFIX)

		ip := ""
		if isLAN {
			ip = strings.TrimPrefix(p.Name, SYSTEM_PDF_NETWORK_PRINTER_PREFIX)
		}
...
func AddLanOfficePrinter(ip string) error {
	...
	printerName := fmt.Sprintf("%s%s", SYSTEM_PDF_NETWORK_PRINTER_PREFIX, ip)
