...
type btAddr struct{ mac string; ch int }
...
func (c *windowsBTConn) SetReadDeadline(t time.Time) error {
	...
	return c.setTimeoutMS(syscall.SO_RCVTIMEO, ms)
}
...
func (c *windowsBTConn) SetWriteDeadline(t time.Time) error {
	...
	return c.setTimeoutMS(syscall.SO_SNDTIMEO, ms)
}
...
func dialRFCOMM(mac string, channel int) (net.Conn, error) {
	...
	r, _, e := procSocket.Call(afBTH, syscall.SOCK_STREAM, bthProtoRFCOMM)
...
func ScanBluetoothPrinters() ([]BluetoothPrinterInfo, error) {
	...
	if handle == invalidHandle || handle == 0 {
		if e == syscall.ERROR_NO_MORE_ITEMS {
			return nil, nil
		}
	...
	for {
		...
		// Check for SPP UUID (Serial Port Profile).
		if ch, err := sdpDiscoverChannel(mac); err == nil && ch > 0 {
			...
			logger.Infof("BT/Windows: %s (%s) has SPP — added as printer", name, mac)
		}
