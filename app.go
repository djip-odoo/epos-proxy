...
func (a *App) AddBluetoothPrinter(mac, name string) error {
	...
	if err := a.config.AddBluetoothPrinter(mac, name, 0); err != nil {
		return fmt.Errorf("failed to save Bluetooth printer: %v", err)
	}

	logger.Infof("Bluetooth printer added: %s (%s)", mac, name)
	return nil
}
...
func (a *App) ConfirmRemoveBluetoothPrinter(mac string) (bool, error) {
	...
	dialog := application.Get().Dialog.Question().
		SetTitle("Remove Printer").	
		SetMessage(fmt.Sprintf("Are you sure you want to remove the Bluetooth printer %s?", mac))
	dialog.AddButton("Cancel").SetAsCancel().OnClick(func() {
		resultChan <- false
	})
	dialog.AddButton("Confirm").SetAsDefault().OnClick(func() {
	...
	if confirmed {
		if err := a.config.RemoveBluetoothPrinter(mac); err != nil {
			return false, fmt.Errorf("failed to remove Bluetooth printer: %w", err)
		}
		logger.Infof("Bluetooth printer removed successfully: %s", mac)
		return true, nil
	}
