...
func dialRFCOMMPlatform(mac string, cachedChannel int) (net.Conn, error) {
	...
	if b, ok := BTManager.cache.get(mac); ok {
		if b.DevPath == "raw" {
			logger.Debugf("BT/RFCOMM: cache hit for raw socket connection to %s on channel %d", mac, b.Channel)
	...
	if err != nil {
		...
		sdpChannel = ch
		logger.Debugf("BT/RFCOMM: SDP discovered channel %d for %s", ch, mac)
	}
	...
	if sdpChannel > 0 && sdpChannel != cachedChannel {
		if conn, err := tryRFCOMMDevice(mac, sdpChannel); err == nil {
			logger.Debugf("BT/RFCOMM: connected successfully on SDP channel %d for %s", sdpChannel, mac)
			return conn, nil
		} else {
...
// executes `rfcomm bind <index> <MAC> <channel>` and waits up to
