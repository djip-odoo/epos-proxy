...
func scanPairedPrinters() ([]BluetoothPrinterInfo, error) {
	...
	for _, line := range strings.Split(strings.TrimSpace(string(out)), "\n") {
		...
		mac := util.NormalizeAddress(parts[1])
		if seen[mac] {
			continue
		}
		...
		seen[mac] = true
		devices = append(devices, BluetoothPrinterInfo{Address: util.NormalizeAddress(mac), Name: name, Device: "printer"})
	}
...
func ensureRFCOMMDevice(mac string, channel int) (*rfcommBinding, error) {
	...
	if existing, ok := findExistingRFCOMMDevice(mac); ok {
		...
		logger.Infof("BT/RFCOMM: reusing existing device %s for %s (channel %d)", existing.DevPath, mac, existing.Channel)
...
func listRFCOMMBindings() (map[int]string, error) {
	...
	for _, line := range strings.Split(string(out), "\n") {
		...
		bindings[idx] = util.NormalizeAddress(fields[0])
	}
