...
var (
	rfcommBindDisabledUntil time.Time
	rfcommBindDisabledMu    sync.RWMutex
	rfcommBindCooldown      = 30 * time.Second
)

func isRFCOMMBindDisabled() bool {
	rfcommBindDisabledMu.RLock()
	defer rfcommBindDisabledMu.RUnlock()
	return time.Now().Before(rfcommBindDisabledUntil)
}

// disableRFCOMMBind temporarily disables rfcomm binding attempts for a cooldown period (30s)
// instead of permanently disabling it for the process lifetime.
func disableRFCOMMBind() {
	disableRFCOMMBindFor(rfcommBindCooldown)
}

// disableRFCOMMBindFor temporarily disables rfcomm binding attempts for duration d.
func disableRFCOMMBindFor(d time.Duration) {
	rfcommBindDisabledMu.Lock()
	defer rfcommBindDisabledMu.Unlock()
	until := time.Now().Add(d)
	if until.After(rfcommBindDisabledUntil) {
		rfcommBindDisabledUntil = until
	}
}

// resetRFCOMMBindDisabled resets the cooldown.
func resetRFCOMMBindDisabled() {
	rfcommBindDisabledMu.Lock()
	defer rfcommBindDisabledMu.Unlock()
	rfcommBindDisabledUntil = time.Time{}
}
...
func dialRFCOMMPlatform(mac string, cachedChannel int) (net.Conn, error) {
	...
	for _, ch := range []int{1, 2, 3, 4, 5, 6, 7, 8} {
		...
		logger.Errorf("BT/RFCOMM: channel %d probe failed for %s", ch, mac)
	}
...
func tryRFCOMMDevice(mac string, channel int) (net.Conn, error) {
	if b, ok := BTManager.cache.get(mac); ok && b != nil {
		if b.Channel == channel && b.DevPath == "raw" {
			...
			if err != nil {
				...
				BTManager.deleteBindingIfMatching(mac, b)
				return nil, err
			}
			return conn, nil
		}
...
func ensureRFCOMMDevice(mac string, channel int) (*rfcommBinding, error) {
	...
	if isRFCOMMBindDisabled() {
		return nil, fmt.Errorf("BT/RFCOMM: rfcomm bind is temporarily in backoff cooldown on this system")
	}
	...
	if existing, ok := findExistingRFCOMMDevice(mac); ok {
		if existing.Channel == 0 || existing.Channel == channel {
...
func bindRFCOMM(mac string, channel int) (*rfcommBinding, error) {
	...
	if err != nil {
		...
		if strings.Contains(outStr, "Operation not permitted") ||
			...
			disableRFCOMMBindFor(2 * time.Minute)
